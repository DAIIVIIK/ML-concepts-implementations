...
class GradientDescent:
    ...
    def Model_training(self):
        ...
        for i in range(400):
            ...
            self.W0 -= self.rate*diff_w0
            self.W1 -= self.rate*diff_w1
            self.W2 -= self.rate*diff_w2
            if i%20==0:
                X_axis.append(i)
...
if __name__ == '__main__':
    ...
    Y_pred = gd.Predict()
    #print (Y_pred)
    print(gd.W0,gd.W1,gd.W2)
    print(RMSE(Y_pred,Y_actual))
    print(r2_score(gd.Y_test,Y_pred ))
    sequence_containing_x_vals = list(np.transpose(gd.X_test)[0])
    sequence_containing_y_vals = list(np.transpose(gd.X_test)[1])
    sequence_containing_z_vals = list(gd.Y_test)
    fig = plt.figure()
    ...
    X,Y = np.meshgrid(x,y)
    Z = gd.W0 + (gd.W1 * X) + (gd.W2 * Y)
    ax.plot_surface(X, Y, Z)
    ax.set_xlabel('Longitude', fontsize=10)
    ax.set_ylabel('Latitude', fontsize=10)
    ax.set_zlabel('Altitude', fontsize=10)
    fig.savefig("ActualvsPredictedforGD.png")
    ...
    #plt3d.plot_surface(gd.W0, gd.W1, gd.W2, alpha=0.2)
